...
func New(config *rest.Config, options Options) (Manager, error) {
	// Options  Controller v1alpha1.ControllerConfigurationSpec 是控制并发度以及同步超时时间的
	// Set default values for options fields
	// 设置一些默认的配置,这里可能包含一些默认的方法
	options = setOptionsDefaults(options)
	// 创建cluster  里面包含了一些与 cluster 通讯的必要组件
	...
	// 重点关注  runnables
	// 是一些可以运行的对象,这些对象被manager 所管理,来完成不同的任务
